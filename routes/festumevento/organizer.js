...
router.post('/savedeposite', helper.authenticateToken, async (req, res) => {
    ...
    if (req.token.superadminid && mongoose.Types.ObjectId.isValid(req.token.superadminid)) {
        ...
        if (superadmin) {
                        ...
                        if (interestper && !(isNaN(interestper)) && (parseFloat(interestper) > 0)) {
                            ...
                            if(dateObject.toString() !== 'Invalid Date'){
                                if(transactionid && transactionid.trim() != ''){
                                    ...
                                    await primary.model(constants.MODELS.organizers, organizerModel).findByIdAndUpdate(organizerid, {isdepositereceived : true, deposite : obj});
            ...
            return responseManager.badrequest({ message: 'Invalid token to save deposite, please try again' }, res);
        }
    } else {
        return responseManager.badrequest({ message: 'Invalid token to save deposite, please try again' }, res);
    }
});
router.post('/returndeposite', helper.authenticateToken, async (req, res) => {
    res.setHeader('Access-Control-Allow-Origin', '*');
    if (req.token.superadminid && mongoose.Types.ObjectId.isValid(req.token.superadminid)) {
        let superadmin = await primary.model(constants.MODELS.superadmins, superadminModel).findById(req.token.superadminid).lean();
        if (superadmin) {
            ...
            if (organizerid && organizerid != '' && mongoose.Types.ObjectId.isValid(organizerid)) {
                let organizerData = await primary.model(constants.MODELS.organizers, organizerModel).findById(organizerid).lean();
                if (organizerData) {
                    await primary.model(constants.MODELS.organizers, organizerModel).findByIdAndUpdate(organizerid, {isdepositereceived : false, deposite : {}});
                    let organizerDataFinal = await primary.model(constants.MODELS.organizers, organizerModel).findById(organizerid).populate({
                        path: 'agentid',
                        model: primary.model(constants.MODELS.agents, agentModel),
                        select: 'name email mobile country_code'
            ...
            return responseManager.badrequest({ message: 'Invalid token to return deposite, please try again' }, res);
        }
    } else {
        return responseManager.badrequest({ message: 'Invalid token to return deposite, please try again' }, res);
    }
...
router.post('/', helper.authenticateToken, async (req, res) => {
    ...
    if (req.token.superadminid && mongoose.Types.ObjectId.isValid(req.token.superadminid)) {
        ...
        let superadmin = await primary.model(constants.MODELS.superadmins, superadminModel).findById(req.token.superadminid).lean();
        if (superadmin) {
            let totalorganizers = parseInt(await primary.model(constants.MODELS.organizers, organizerModel).countDocuments({}));
...
router.post('/remove', helper.authenticateToken, async (req, res) => {
    ...
    if (req.token.superadminid && mongoose.Types.ObjectId.isValid(req.token.superadminid)) {
        ...
        if (superadmin) {
            if (organizerid && organizerid != '' && mongoose.Types.ObjectId.isValid(organizerid)) {
                await primary.model(constants.MODELS.organizers, organizerModel).findByIdAndUpdate(organizerid, { status: false, is_approved: false });
...
router.post('/getone', helper.authenticateToken, async (req, res) => {
    ...
    if (req.token.superadminid && mongoose.Types.ObjectId.isValid(req.token.superadminid)) {
        ...
        if (superadmin) {
            if (organizerid && organizerid != '' && mongoose.Types.ObjectId.isValid(organizerid)) {
                let organizerData = await primary.model(constants.MODELS.organizers, organizerModel).findById(organizerid).populate({
                    path: 'agentid',
                    model: primary.model(constants.MODELS.agents, agentModel),
                    select: 'name email mobile country_code'
                }).lean();
