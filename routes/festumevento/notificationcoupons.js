...
router.post('/', helper.authenticateToken, async (req, res) => {
    ...
    if (req.token.superadminid && mongoose.Types.ObjectId.isValid(req.token.superadminid)) {
        ...
        if(superadmin){
            let totalnotificationcoupons = parseInt(await primary.model(constants.MODELS.notificationcoupons, notificationcouponModel).countDocuments({}));
            primary.model(constants.MODELS.notificationcoupons, notificationcouponModel).paginate({
                ...
                notificationcoupons.totalnotificationcoupons = totalnotificationcoupons;
...
router.post('/remove', helper.authenticateToken, async (req, res) => {
    ...
    if (req.token.superadminid && mongoose.Types.ObjectId.isValid(req.token.superadminid)) {
        ...
        if(superadmin){
            if(notificationcouponid && notificationcouponid != '' && mongoose.Types.ObjectId.isValid(notificationcouponid)){
                await primary.model(constants.MODELS.notificationcoupons, notificationcouponModel).findByIdAndUpdate(notificationcouponid, {status : false});
