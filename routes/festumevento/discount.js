...
router.post('/', helper.authenticateToken, async (req, res) => {
    ...
    if (req.token.superadminid && mongoose.Types.ObjectId.isValid(req.token.superadminid)) {
        ...
        let totalDiscounts = parseInt(await primary.model(constants.MODELS.discounts, discountModel).countDocuments({}));
        if(superadmin){
...
router.post('/remove', helper.authenticateToken, async (req, res) => {
    ...
    if (req.token.superadminid && mongoose.Types.ObjectId.isValid(req.token.superadminid)) {
        ...
        if(superadmin){
            if(discountid && discountid != '' && mongoose.Types.ObjectId.isValid(discountid)){
                await primary.model(constants.MODELS.discounts, discountModel).findByIdAndUpdate(discountid, {status : false});
