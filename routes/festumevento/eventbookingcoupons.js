...
router.post('/', helper.authenticateToken, async (req, res) => {
    ...
    if (req.token.superadminid && mongoose.Types.ObjectId.isValid(req.token.superadminid)) {
        ...
        if(superadmin){
            let totaleventbookingcoupons = parseInt(await primary.model(constants.MODELS.eventbookingcoupons, eventbookingcouponModel).countDocuments({}));
...
router.post('/remove', helper.authenticateToken, async (req, res) => {
    ...
    if (req.token.superadminid && mongoose.Types.ObjectId.isValid(req.token.superadminid)) {
        ...
        if(superadmin){
            if(eventbookingcouponid && eventbookingcouponid != '' && mongoose.Types.ObjectId.isValid(eventbookingcouponid)){
                await primary.model(constants.MODELS.eventbookingcoupons, eventbookingcouponModel).findByIdAndUpdate(eventbookingcouponid, {status : false});
