...
router.post('/', helper.authenticateToken, async (req, res) => {
    ...
    if (req.token.superadminid && mongoose.Types.ObjectId.isValid(req.token.superadminid)) {
        ...
        if(superadmin){
            let totalpromotionplans = parseInt(await primary.model(constants.MODELS.promotionplans, promotionplanModel).countDocuments({}));
            primary.model(constants.MODELS.promotionplans, promotionplanModel).paginate({
                ...
                plans.totalpromotionplans = totalpromotionplans;
...
router.post('/remove', helper.authenticateToken, async (req, res) => {
    ...
    if (req.token.superadminid && mongoose.Types.ObjectId.isValid(req.token.superadminid)) {
        ...
        if(superadmin){
            if(planid && planid != '' && mongoose.Types.ObjectId.isValid(planid)){
                await primary.model(constants.MODELS.promotionplans, promotionplanModel).findByIdAndUpdate(planid, {status : false});
